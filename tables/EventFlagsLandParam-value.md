Ninji's exports: [HTML](https://wuffs.org/acnh/bcsv_160/html/EventFlagsLandParam.html), [CSV](https://wuffs.org/acnh/bcsv_160/csv/EventFlagsLandParam.csv), [JSON](https://wuffs.org/acnh/bcsv_160/json/EventFlagsLandParam.json)
...
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | HetHostOnly,自分の村のみ書き換え可(自動通信あり) | 1 | 119 | 'FlightBalloonEnable' | '空飛ぶ風船|発生してよいか？' | 1 | 0 | 
...
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | HetHostOnly,自分の村のみ書き換え可(自動通信あり) | 1 | 135 | 'MessageBottleAvailable' | 'メッセージボトル漂着が解禁' | 1 | 0 | 
...
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 290 | 'CampFireRemoved' | '広場のキャンプファイアー撤去（実際の撤去とは連動してません）' | 1 | 0 | 
...
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 336 | 'BCAT_EventFlag_004' | 'ハーベスト解禁' | 1 | 0 | 
...
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 372 | 'GrowUpAfterPatch1_6' | '1.6適用して成長処理をした' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 373 | 'IslandProducedByPlayerMoving' | 'プレイヤーだけ引越し|プレイヤーだけ引越しによって作られた島か？' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 374 | 'EventObjFlag0' | 'イベントオブジェフラグ0' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | SpNpcOnly,特殊NPCの会話中のみ(自動通信あり) | 4 | 377 | 'HarvestProgress' | 'ハーベスト｜完成した料理の数（島単位）' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 378 | 'PlayerMovingEnableShopMaterialCollect' | 'プレイヤーだけ引越し|お店の資材集め解禁していいか？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 379 | 'SecondPublicAnnouncement' | '2回目島内放送の発生が必要か？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadOnly,○参照×書き込み | ReadOnly,参照のみ可能 | 1 | 380 | 'HarvestAfterTerm' | 'ハーベスト｜アフター期間か？' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | HetHostOnly,自分の村のみ書き換え可(自動通信あり) | 1 | 381 | 'FinishTreasureQuestByRollback' | '宝探しクエストが通信ロールバックで終了した' | 1 | 1 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | SpNpcOnly,特殊NPCの会話中のみ(自動通信あり) | 1 | 382 | 'HarvestHQDish1Island' | 'ハーベスト｜島の誰かが料理１クラムチャウダーの隠し食材納めた？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | SpNpcOnly,特殊NPCの会話中のみ(自動通信あり) | 1 | 383 | 'HarvestHQDish2Island' | 'ハーベスト｜島の誰かが料理２パンプキンパイの隠し食材納めた？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | SpNpcOnly,特殊NPCの会話中のみ(自動通信あり) | 1 | 384 | 'HarvestHQDish3Island' | 'ハーベスト｜島の誰かが料理３グラタンの隠し食材納めた？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | EnableReadWrite,○参照○書き込み | SpNpcOnly,特殊NPCの会話中のみ(自動通信あり) | 1 | 385 | 'HarvestHQDish4Island' | 'ハーベスト｜島の誰かが料理４サカナのムニエルの隠し食材納めた？' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 386 | 'IsDisclosedMyDream' | 'ゆめみ|現在、自分の島の夢をおまかせに公開中か？' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 387 | 'HarvestTukIngredient2_2' | 'ハーベスト｜必要食材２－２' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 388 | 'HarvestTukIngredient3_1' | 'ハーベスト｜必要食材３－１' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 389 | 'HarvestTukIngredient3_2' | 'ハーベスト｜必要食材３－２' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 390 | 'HarvestTukIngredient4_2' | 'ハーベスト｜必要食材４－２' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 391 | 'HarvestTukHideIngredient2_1' | 'ハーベスト｜隠し食材２－１' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 19 | 392 | 'HarvestTukHideIngredient2_2' | 'ハーベスト｜隠し食材２－２' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 5 | 393 | 'ReputaionMyIsland' | 'ゆめみ｜現在の自分の島の評判値' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 9999 | 396 | 'LastPlayXmasYear' | '最後にプレイしたクリスマスの年' | 1 | 0 | 
| Pre,日またぎ成長処理前（経過日数が0以外） | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 397 | 'ShopSelectChristmasFtr' | 'クリスマスおもちゃ家具抽選を今日したか' | 1 | 0 | 
| None,クリアしない | 0 | EnableReadWrite,○参照○書き込み | ReadOnly,参照のみ可能 | 1 | 398 | 'IsUploadDisclosedMyDream' | 'ゆめみ|オマカセ公開で初回更新しなかったか？' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 399 | 'TkkFirstLiveNow' | 'とたけけ|初ライブステージか？' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 400 | 'ChristmasFtrFirstRound' | 'クリスマス｜おもちゃ家具の商品抽選が1巡したか' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 9999 | 401 | 'HarvestFestivalAddBbsYear' | 'ハーベストフェスティバル予告の掲示板書き込みをした年' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 9999 | 403 | 'XmasEveAddBbsYear' | 'クリスマス予告の掲示板書き込みをした年' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 404 | 'BCAT_EventFlag_005' | 'クリスマス準備期間解禁' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 65535 | 405 | 'RandomKey5' | 'ランダムキーe' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 406 | 'ShopSocksFlag' | 'かべかけソックス当選済み' | 1 | 0 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 65535 | 409 | 'RandomKey6' | 'ランダムキーf' | 1 | 1 | 
| None,クリアしない | 0 | DisableAccess,×参照×書き込み | ReadOnly,参照のみ可能 | 1 | 412 | 'BCAT_EventFlag_006' | 'クリスマスイブ解禁' | 1 | 0 | 
